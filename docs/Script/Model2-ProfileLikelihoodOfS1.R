local.prob.for.ll.model2 <- function(par, fixedS1, effort, catchability.scaling.factor){
    ...
    M <- par[2]
    selectivity.at.age <-rbind( outer(rep(1,6), c(fixedS1, rep(1,5))),
...
local.ll.model2 <- function(par, fixedS1, catch, effort, catchability.scaling.factor){
    ...
    P <- local.prob.for.ll.model2(par, fixedS1, effort, catchability.scaling.factor)
...
lower.bound <- c("q" = 5e-2, "M" = 1e-2, "s2" = 1e-3); upper.bound <- c("q" = 15, "M" = 1, "s2" = 1)
...
for( S1 in seq(0.07,0.13, length = 1e3)){
print(paste("Counter=", counter))

result2 <- optim(par = c(1.0, 0.5, 0.1), fn = local.ll.model2, catch = nb.at.age.wgt, effort = effort, fixedS1 = S1,
	catchability.scaling.factor = csf, method = c("L-BFGS-B"),
       	lower = lower.bound, upper = upper.bound, hessian = TRUE)

my.res[counter, "s1"] <- S1
my.res[counter, "log.lik"] <- result2$value
my.res[counter, c("q","M", "s2")] <- result2$par
counter <- counter +1;
#print(my.res)
}

write.csv(my.res, paste("Results/Data/Model2-ProfileLikelihoodOfS1-", format(Sys.time(), "%b%d%Y-%H-%M-%S"), ".csv", sep=""))

with(my.res, plot(s1, log.lik, type = "l"))
