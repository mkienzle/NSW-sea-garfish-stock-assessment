local.prob.for.ll.model2 <- function(par, fixedS2, effort, catchability.scaling.factor){
    ...
    selectivity.at.age <-rbind( outer(rep(1,6), c(par[3], rep(1,5))),
    		                outer(rep(1,nrow(effort)-6), c(fixedS2, rep(1,5))))
...
local.ll.model2 <- function(par, fixedS2, catch, effort, catchability.scaling.factor){
    ...
    P <- local.prob.for.ll.model2(par, fixedS2, effort, catchability.scaling.factor)
...
lower.bound <- c("q" = 5e-2, "M" = 1e-2, "s1" = 1e-3); upper.bound <- c("q" = 15, "M" = 1, "s1" = 1)
...
for( S2 in seq(0.01,0.06, length = 1e3)){
print(paste("Counter=", counter))

result2 <- optim(par = c(1.0, 0.5, 0.1), fn = local.ll.model2, catch = nb.at.age.wgt, effort = effort, fixedS2 = S2,
	catchability.scaling.factor = csf, method = c("L-BFGS-B"),
       	lower = lower.bound, upper = upper.bound, hessian = TRUE)

my.res[counter, "s2"] <- S2
my.res[counter, "log.lik"] <- result2$value
my.res[counter, c("q","M", "s1")] <- result2$par
counter <- counter +1;
#print(my.res)
}

write.csv(my.res, paste("Results/Data/Model2-ProfileLikelihoodOfS2-", format(Sys.time(), "%b%d%Y-%H-%M-%S"), ".csv", sep=""))

with(my.res, plot(s2, log.lik, type = "l"))
